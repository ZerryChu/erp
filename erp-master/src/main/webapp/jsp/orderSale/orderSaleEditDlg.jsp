...
<script type="text/javascript">
	...
	$(function() {
		...
		$dg
				.datagrid({
					url : "orderSale/orderSaleAction!findOrderSaleLineList.action?orderSaleId="
							+ $("#tempId").val(),
					width : 'auto',
					...
					columns : [ [ {
						...
						field : 'price',
						title : '单价(或最近成交价)',
						width : parseInt($(this).width() * 0.09),
						align : 'left',
						editor : {
							type : 'numberbox',
							options : {
		...
		$cb
				.combogrid({
					url : "cst/cstAction!findCustomerListNoPage.action",
					width : 171,
					panelWidth : 400,
					panelHeight : 300,
					rownumbers : true,
					...
					onSelect : function(rowIndex, rowData) {
						...
						$("#customerContact")
								.combobox(
										{
											width : 171,
											url : "cstContact/cstContactAction!findCustomerContactListCombobox.action?customerId="
													+ rowData.customerId,
											valueField : 'name',
											textField : 'name',
		...
		$("#form").form({
			url : "orderSale/orderSaleAction!persistenceOrderSale.action",
			onSubmit : function() {
				...
				.bind(
						...
						function(event) {
							...
							if (keyDown == "keyDown") {
								...
								case (38):
									var index = $dg.datagrid("getRowIndex", $dg
											.datagrid("getSelected"));
									var temp = $dg.datagrid('validateRow',
											index);
									if (temp) {
										if (index > 0) {
											$dg
											...
											$(ed.target).focus();
											var editemName = $dg.datagrid(
													'getEditor', {
														index : index - 1,
														field : 'itemName'
													});
											$(editemName.target).attr(
													'disabled', false);
											var edunit = $dg.datagrid(
													'getEditor', {
														index : index - 1,
														field : 'unit'
													});
											$(edunit.target).attr('disabled',
													false);
											var edamount = $dg.datagrid(
													'getEditor', {
														index : index - 1,
														field : 'amount'
													});
											$(edamount.target).attr('disabled',
													true);
											var edtaxAmount = $dg.datagrid(
													'getEditor', {
														index : index - 1,
														field : 'taxAmount'
													});
											$(edtaxAmount.target).attr(
													'disabled', true);
								...
								case (40):
									...
									if (index == (length - 1)) {
										...
										$(editemName.target).attr('disabled',
												false);
										var edunit = $dg.datagrid('getEditor',
												{
													index : index + 1,
													field : 'unit'
												});
										...
										$(edtaxAmount.target).attr('disabled',
												true);

										var edorderQty = $dg.datagrid(
												'getEditor', {
													index : index,
													field : 'orderQty'
												});
										if (edorderQty) {
											var edorderQtyValue = $(
													edorderQty.target)
													.numberbox("getValue");
											var edprice = $dg.datagrid(
													'getEditor', {
														index : index,
														field : 'price'
													});
											var edpriceValue = $(edprice.target)
													.numberbox("getValue");
											var eddiscountNo = $dg.datagrid(
													'getEditor', {
														index : index,
														field : 'discountNo'
													});
											var eddiscountNoValue = $(
													eddiscountNo.target)
													.numberbox("getValue");
											var edtaxNo = $dg.datagrid(
													'getEditor', {
														index : index,
														field : 'taxNo'
													});
											var edtaxNoValue = $(edtaxNo.target)
													.numberbox("getValue");
											if (eddiscountNoValue == 0) {
												var v1 = parseFloat((edpriceValue * edorderQtyValue)
														.toFixed(2));
												var v2 = parseFloat((edpriceValue
														* edorderQtyValue * (parseFloat(edtaxNoValue * 0.01)))
														.toFixed(2));
												amount += v1;
												taxAmount += v2;
												var edamount = $dg.datagrid(
												...
												var v3 = parseFloat((edpriceValue
														* edorderQtyValue * (parseFloat(eddiscountNoValue)))
														.toFixed(2));
												var v4 = parseFloat((parseFloat(eddiscountNoValue)
														* edpriceValue
														* edorderQtyValue * (parseFloat(edtaxNoValue * 0.01)))
														.toFixed(2));
												amount += v3;
	...
	function endEdit() {
		...
		for (var i = 0; i < rows.length; i++) {
			row = rows[i];
			var edorderQty = $dg.datagrid('getEditor', {
				index : i,
				field : 'orderQty'
			});
			if (edorderQty) {
				var edorderQtyValue = $(edorderQty.target)
						.numberbox("getValue");
				...
				var eddiscountNo = $dg.datagrid('getEditor', {
					index : i,
					field : 'discountNo'
				});
				var eddiscountNoValue = $(eddiscountNo.target).numberbox(
						"getValue");
				var edtaxNo = $dg.datagrid('getEditor', {
					index : i,
					field : 'taxNo'
				});
				...
				if (eddiscountNoValue == 0) {
					var v1 = parseFloat((edpriceValue * edorderQtyValue)
							.toFixed(2));
					var v2 = parseFloat((edpriceValue * edorderQtyValue * (parseFloat(edtaxNoValue * 0.01)))
							.toFixed(2));
					//amount+= v1;
					// taxAmount+=v2;
					var edamount = $dg.datagrid('getEditor', {
						index : i,
						field : 'amount'
					});
					$(edamount.target).numberbox("setValue", v1);
					var edtaxAmount = $dg.datagrid('getEditor', {
						index : i,
						field : 'taxAmount'
					});
					$(edtaxAmount.target).numberbox("setValue", v2);
				} else {
					var v3 = parseFloat((edpriceValue * edorderQtyValue * (parseFloat(eddiscountNoValue)))
							.toFixed(2));
					var v4 = parseFloat((parseFloat(eddiscountNoValue)
							* edpriceValue * edorderQtyValue * (parseFloat(edtaxNoValue * 0.01)))
							.toFixed(2));
					//amount+= v3;
					//taxAmount+=v4;
					var edamount = $dg.datagrid('getEditor', {
			...
			if (row.discountNo == 0) {
				...
				taxAmount += parseFloat((row.price * row.orderQty * (parseFloat(row.taxNo * 0.01)))
						.toFixed(2));
			} else {
				// 有折扣
				amount += parseFloat((row.price * row.orderQty * (parseFloat(row.discountNo)))
						.toFixed(2));
				taxAmount += parseFloat((parseFloat(row.discountNo) * row.price
						* row.orderQty * (parseFloat(row.taxNo * 0.01)))
						.toFixed(2));
			}
		}
	...
	function addRows() {
		...
		if (isValid) {
			if (suplierId) {
				...
				var editemName = $dg.datagrid('getEditor', {
					index : ll - 1,
					field : 'itemName'
				});
		...
		<form id="form" method="post">
			<div title="基本资料" data-options="iconCls:'icon-cstbase'"
				...
				<fieldset>
					...
					<input name="orderSaleId" id="orderSaleId" type="hidden" /> <input
					...
					<table class="table">
						<tr>
							...
							<th>交易状态</th>
							<td><input name="isOngoing" id="isOngoing" type="text"
						...
						<tr>
							...
							<td><input name="customerId" id="customerId" type="text"
								...
								name="customerMyid" id="customerMyid" type="hidden" /></td>
							<th>客户名称</th>
							<td><input id="customerName" name="customerName" type="text"
								class="easyui-textbox easyui-validatebox"
								style="background-color: #cccccc;" readonly /></td>
							<th>联系人</th>
							<td><input id="customerContact" name="customerContact"
						...
						<tr>
							...
							<th>销售部门</th>
							<td><input id="saleOrganizationId" name="saleOrganizationId"
								type="text" class="easyui-textbox easyui-validatebox" /><input
								id="saleOrganizationName" name="saleOrganizationName"
						...
						<tr>
							...
							<th>客户单号</th>
							<td><input id="customerOrderNo" name="customerOrderNo"
						...
						<tr>
							<th>业务员</th>
							<td><input id="saleId" name="saleId" type="text"
								class="easyui-textbox" /><input id="saleName" name="saleName"
								type="hidden" class="easyui-textbox" /></td>
							<th>立账方式</th>
							<td>
								<!--  <input id="taxAmount1" name="taxAmount1" type="text" class="easyui-textbox"/>-->
							</td>
							<th>备注</th>
							<td colspan="2">
								<!-- <input id="description" name="description" type="text" class="easyui-textbox"/>-->
							</td>
						</tr>
					</table>
				</fieldset>
